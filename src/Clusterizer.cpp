	...
	//std::cout << "========cluster" << clusterID  << "=====================" << "\n";	
	  ...
	  int cL1id= clusterHits[c].l1id;
	    ...
	    //std::cout << "Hit: " << (*chip).second[h].col << "-" << (*chip).second[h].row << "\n";
   	    if ( ( abs(cCol - (*chip).second[h].col) <= (int)cdCol  )  && ( abs(cRow - (*chip).second[h].row ) <= (int)cdRow ) && 
	         ( abs(cBcid- (*chip).second[h].bcid)<10000)  && ( abs(cL1id- (*chip).second[h].l1id) <= (int)lv1diff ) )
   	    {
