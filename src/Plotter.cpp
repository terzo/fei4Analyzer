...
void Plotter::fillClusterPlots(Clusterizer::clusterMapDef &clusterMap, double noise)
{
    ...
    if(clusterToT_.count(clusterMap.begin()->second.begin()->first) == 0 && isQuad_)
    {
        ...
        clusterMap_all_             = addPlot(clusterMap_all_           ,"clusterMap_all"       ,cols*2,0,cols*2,rows*2,0,rows*2);
...
//==================================================================
void Plotter::setNRowCols(const int num_rows, const int num_cols)
{
    cols_ = num_cols;
    rows_ = num_rows;
}
...
{ 
    ...
    for(std::map<int, TH1I*>::iterator chip=clusterToT_.begin(); chip!=clusterToT_.end(); ++chip)
    {
        ...
        ss_ << "lvl1";
        dir->mkdir(ss_.str().c_str())->cd();
        for(std::map<int, TH2I*>::iterator cs=lvl1_[(*chip).first].begin(); cs!=lvl1_[(*chip).first].end(); ++cs)
        {
            cs->second->Write();
        }
    }
